  ...
  for (y=0; y<height; y++)
    {
      for (x=0; x<width; x++)
	{
	  Uint8 curr = p_bitmap[ (y * byte_w + x/8) + 2];
	  int bit_on = ( curr & (0x80 >> (x & 7)) ) != 0;

	  sdlfe_putpixel(x + xpos, y + ypos, bit_on ^ mode);
	}
    }
}
...
void fe_draw_bitmap(const uint8_t *p_bitmap, uint8_t x, uint8_t y, uint8_t inverted)
{
  uint8_t byte_w;
  uint8_t w;
  uint8_t h;
...
void fe_print_xy(uint8_t x, uint8_t y, uint8_t mode, char *p_str)
{
  ...
  if ( !(p_text = TTF_RenderText_Solid( (mode & FE_FONT_SMALL) ? p_font_small : p_font, p_str, clr)) )
...
void fe_init()
{
  ...
  if ( !(p_font_small = TTF_OpenFont("arial.ttf", 9)) )
    {
      char buf[MAX_COMMAND_DIR_LEN*2];
