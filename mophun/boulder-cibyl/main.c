 ...
 *
 * $Id: main.c 9085 2006-07-19 10:41:58Z ska $
 *
 ********************************************************************/
#include <cibyl.h>
...
static void game_center_map(game_t *p_game)
{
  ...
  NOPH_Layer_setPosition(p_game->tilemap, p_game->bg_x, p_game->bg_y);
}
...
static bool_t show_tile(game_t *p_game, point_t tile_pt)
{
  ...
  NOPH_TiledLayer_setCell(p_game->tilemap, tile_pt.x, tile_pt.y, tile);
...
static void game_erase_view_buffer(game_t *p_game)
{
  ...
  for (y=y_start; y<=y_end; y++)
      ...
      for (x=x_start; x<=x_end; x++)
	{
	  ...
	  tile_t tile = NOPH_TiledLayer_getCell(p_game->tilemap, x,y);

	  if (tile != TILE_UNDISCOVERED &&
	      tile != TILE_EMPTY &&
	      tile < TILE_MAX)
	    {
	      NOPH_TiledLayer_getCell(p_game->tilemap, x,y, tile + TILE_MAX);
	      if (MASK_TILE_IS_ELEM(mask_tile) &&
...
static void game_init(game_t *p_game)
{
  ...
  p_game->small_font.width = 4;
  p_game->small_font.height = 6;
  p_game->small_font.bpp = 1;
  p_game->small_font.palindex = 1;
  p_game->small_font.chartbl = SMALL_FONT+16; /* Which characters are present in the font? */
  p_game->small_font.fontdata = SMALL_FONT+16+256; /* The pixel data */

  p_game->pp_sprite_frames = unpack_sprite_frames(p_buf, SPRITE_FRAMES_UNPACKED_SIZE, N_FRAMES);
  free(p_buf);

  if ( !(p_buf = (void*)get_resource(LOGO, LOGO_SIZE)) )
    error_msg("Loading the title screen failed\n");
  if ( !(p_game->p_title = (SPRITE*)unpack_data(p_buf, LOGO_UNPACKED_SIZE)) )
    error_msg("Unpacking the title failed\n");
  free(p_buf);

  if ( !(p_buf = (void*)get_resource(BG_TILES, BG_TILES_SIZE)) )
    error_msg("Loading the tiles failed\n");
  if ( !(p_game->p_tiles = (uint8_t*)unpack_data(p_buf, BG_TILES_UNPACKED_SIZE)) )
    error_msg("Unpacking the tiles failed\n");
  free(p_buf);
...
static void game_fini(game_t *p_game)
{
  ...
  NOPH_delete(p_game->title);
  NOPH_delete(p_game->tiles);
  free(p_game->pp_sprite_frames);

  /* FIXME: Display finish screen */
  NOPH_delete(p_game->tilemap);
}
  ...
  while(p_game->game_on)
    {
      ...
      before = NOPH_System_currentTimeMilis(); /* Get the current ms ticks */
	...
	{
	  uint32_t fire = keys & (FIRE_PRESSED | GAME_A_PRESSED);
	  /* Move the player around */
	  if (keys & LEFT_PRESSED)
	    player_handle_input(p_game, LEFT, fire);
	  else if (keys & RIGHT_PRESSED)
	    player_handle_input(p_game, RIGHT, fire);
	  else if (keys & UP_PRESSED)
	    player_handle_input(p_game, UP, fire);
	  else if (keys & DOWN_PRESSED)
	    player_handle_input(p_game, DOWN, fire);
	}
      if (keys & KEY_SELECT)
	return;
      ...
      if ((p_game->frame_count & 2) == 0)
	{
      ...
      after = NOPH_System_currentTimeMilis(); /* Get the current ms ticks */
      /* Every loop iteration should take about SLEEP_PERIOD, see to that */
      if ((after - before) < SLEEP_PERIOD)
	{
	  NOPH_Thread_sleep( SLEEP_PERIOD - (after-before) );
	}
#if 0
      else
...
int main(int argc, char *argv[])
{
  ...
  game_init(&game);
  menu_init(&main_menu, &game.small_font, main_menu_msgs,
	    0, screen_h/2, screen_w, screen_h);
  menu_init(&options_menu, &game.small_font, options_menu_msgs,
	    0, screen_h/2, screen_w, screen_h);
